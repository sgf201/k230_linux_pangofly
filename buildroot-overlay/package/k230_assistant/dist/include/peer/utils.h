...
#define LOG_PRINT(level_tag, fmt, ...) \
  peer_log(level_tag, strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : __FILE__, __LINE__, fmt, ##__VA_ARGS__)
#else
#define LOG_PRINT(level_tag, fmt, ...) \
  fprintf(stdout, "%s\t%s\t%d\t" fmt "\n", level_tag, strrchr(__FILE__, '/') ? strrchr(__FILE__, '/') + 1 : __FILE__, __LINE__, ##__VA_ARGS__)
#endif
