...
int mbedtls_ssl_handshake_step(mbedtls_ssl_context *ssl)
{
    ...
    // printf("ssl_prepare_handshake_step ret:%d\n",ret);
    if (ret != 0) {
        return ret;
    }

    ret = mbedtls_ssl_handle_pending_alert(ssl);
    // printf("mbedtls_ssl_handle_pending_alert ret:%d\n",ret);
    if (ret != 0) {
        goto cleanup;
    }
    ...
    if (ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT) {
        ...
        switch (ssl->state) {
            ...
            default:
                ...
                ret = mbedtls_ssl_handshake_client_step(ssl);
                // printf("mbedtls_ssl_handshake_client_step ret:%d\n",ret);
#else
                ret = mbedtls_ssl_tls13_handshake_client_step(ssl);
#endif
        }
    ...
    if (ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER) {
        ...
        if (mbedtls_ssl_conf_is_tls12_only(ssl->conf)) {
            ...
            // printf("mbedtls_ssl_handshake_server_step ret:%d\n",ret);
        }
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
    }
#endif
...
int mbedtls_ssl_handshake(mbedtls_ssl_context *ssl)
{
    ...
    while (ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER) {
        ...
        // printf("mbedtls_ssl_handshake_step ret:%d\n",ret);
        if (ret != 0) {
            break;
        }
    }
...
int mbedtls_ssl_parse_certificate(mbedtls_ssl_context *ssl)
{
    ...
    // printf("ssl_parse_certificate_coordinate\n");
...
#if defined(MBEDTLS_SSL_SRV_C)
    // printf("ssl_srv_check_client_no_crt_notification\n");
    ...
    if (chain == NULL) {
        ...
        mbedtls_ssl_send_alert_message(ssl,
                                       MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       MBEDTLS_SSL_ALERT_MSG_INTERNAL_ERROR);
        printf("mbedtls_ssl_send_alert_message\n");
        ret = MBEDTLS_ERR_SSL_ALLOC_FAILED;
        goto exit;
    }
...
crt_verify:
    ...
    ret = ssl_parse_certificate_verify(ssl, authmode,
                                       chain, rs_ctx);
    // printf("ssl_parse_certificate_verify:%d\n",ret);
    if (ret != 0) {
        goto exit;
    }
...
#if !defined(MBEDTLS_SSL_KEEP_PEER_CERTIFICATE)
    {
        ...
        printf("ssl_remember_peer_crt_digest:%d\n",ret);
        if (ret != 0) {
            goto exit;
        }

        ret = ssl_remember_peer_pubkey(ssl, pk_start, pk_len);
        printf("ssl_remember_peer_pubkey:%d\n",ret);
        if (ret != 0) {
            goto exit;
        }
    }
