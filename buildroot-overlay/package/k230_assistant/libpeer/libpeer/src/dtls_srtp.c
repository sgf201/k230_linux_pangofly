...
static void dtls_srtp_key_derivation(void* context, mbedtls_ssl_key_export_type secret_type, const unsigned char* secret, size_t secret_len, const unsigned char client_random[32], const unsigned char server_random[32], mbedtls_tls_prf_types tls_prf_type) {
  ...
  int ret;
  // printf("mbedtls_ssl_key_export_type: %d, secret: %s,secret length: %zu\n",secret_type,secret,secret_len);
  ...
  ret = srtp_create(&dtls_srtp->srtp_in, &dtls_srtp->remote_policy);
  // printf("srtp_in create ret:%d\n",ret);
  // printf("dtls_srtp remote key: %s\n",dtls_srtp->remote_policy.key);
  // printf("dtls_srtp srtp_in addr:%p\n",dtls_srtp->srtp_in);
  ...
  ret = srtp_create(&dtls_srtp->srtp_out, &dtls_srtp->local_policy);
  // printf("srtp_out create ret:%d\n",ret);
  // printf("dtls_srtp local key: %s\n",dtls_srtp->local_policy.key);
  // printf("dtls_srtp srtp_out addr:%p\n",dtls_srtp->srtp_out);
  if (ret != srtp_err_status_ok) {
